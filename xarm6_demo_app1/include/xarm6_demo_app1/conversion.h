...
inline EulerAngle toEulerAngle(xarm6_demo_app1::Quaternion q, EulerOrder order) {
...
inline EulerAngle toEulerAngle(RotationMatrix m, EulerOrder order) {
  if (order == EulerOrder::XYZ) {
    auto sy = m.at(0, 2);
    auto unlocked = std::abs(sy) < 0.99999f; 
    return EulerAngle(
...
inline xarm6_demo_app1::Quaternion toQuaternion(EulerAngle e) {
  ...
  switch (e.order) {
  case EulerOrder::XYZ:
    return xarm6_demo_app1::Quaternion(
      cx * sy * sz + sx * cy * cz,
      -sx * cy * sz + cx * sy * cz,
      cx * cy * sz + sx * sy * cz,
      -sx * sy * sz + cx * cy * cz
...
inline xarm6_demo_app1::Quaternion toQuaternion(RotationMatrix m) {
...
inline RotationMatrix toRotationMatrix(EulerAngle e) {
  auto cx = std::cos(e.x);
  auto sx = std::sin(e.x);
  auto cy = std::cos(e.y);
  auto sy = std::sin(e.y);
...
inline RotationMatrix toRotationMatrix(xarm6_demo_app1::Quaternion q) {
  auto xy2 = q.x * q.y * 2;
  auto xz2 = q.x * q.z * 2;
  auto xw2 = q.x * q.w * 2;
  auto yz2 = q.y * q.z * 2;
