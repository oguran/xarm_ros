...
void CObjListManager::enablePublishTargetTF() {
  {
    std::lock_guard<std::mutex> lock(mtx_enable_);
    enable_publish_target_tf = true;
  }
}

void CObjListManager::disablePublishTargetTF() {
  {
    std::lock_guard<std::mutex> lock(mtx_enable_);
    enable_publish_target_tf = false;
  }
}

void CObjListManager::CameraInfoCallback(const sensor_msgs::CameraInfoConstPtr& msg_cinfo) {
  if (rcv_cinfo_) return;
...
void CObjListManager::ObjPoseListCallback(const srecog_msgs::ObjPoseList& obj_pose_list) {
  ...
  {
    std::lock_guard<std::mutex> lock(mtx_enable_);
    if (!enable_publish_target_tf) return;
  }
