...
CRobotModelStateTest::CRobotModelStateTest()
  : robot_model_loader("robot_description")
{
}

bool CRobotModelStateTest::Test() {
  ...
  // Get Joint Values
  // ^^^^^^^^^^^^^^^^
  // We can retreive the current set of joint values stored in the state for the xarm6.
  ...
  /* Set one joint in the xarm6 outside its joint limit */
  joint_values[0] = 5.57;
  ...
  // Inverse Kinematics
  // ^^^^^^^^^^^^^^^^^^
  // We can now solve inverse kinematics (IK) for the xarm6 robot.
  // To solve IK, we will need the following:
  //
  //  * The desired pose of the end-effector (by default, this is the last link in the "xarm6" chain):
