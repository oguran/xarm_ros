...
CPlanningSceneTest::CPlanningSceneTest()
  : robot_model_loader("robot_description")
{
}


bool CPlanningSceneTest::Test() {
  ...
  // xarm, i.e. we will check whether there are any collisions between
  // the hand and other parts of the body of the robot. We can ask
  // for this specifically by adding the group name "xarm_gripper" to the
  // collision request.

  collision_request.group_name = "xarm_gripper";
  current_state.setToRandomPositions();
  collision_result.clear();
  planning_scene.checkSelfCollision(collision_request, collision_result);
  ...
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~
  //
  // First, manually set the xarm to a position where we know
  // internal (self) collisions do happen. Note that this state is now
  // actually outside the joint limits of the xarm, which we can also
  // check for directly.
  ...
  // have happened at a given configuration of the xarm. We can ask
  ...
  // on the end-effector of the xarm group of the xarm robot. Note the
  ...
  bool state_valid = planning_scene.isStateValid(copied_state, kinematic_constraint_set, PLANNING_GROUP);
